...
def marching_cubes_func(tuple lower, tuple upper, int numx, int numy, int numz, object f, double isovalue, double truncation):
    ...
    verts, faces = c_marching_cubes_func(lower, upper, numx, numy, numz, f, isovalue, truncation)
    verts.shape = (-1, 3)
    faces.shape = (-1, 3)
    return verts, faces
